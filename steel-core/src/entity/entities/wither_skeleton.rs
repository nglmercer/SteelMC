//! WitherSkeleton entity - vanilla `{java_class}`.
...
use steel_registry::vanilla_entity_data::WitherSkeletonEntityData;
...
pub struct WitherSkeletonEntity {
    ...
    mob_base: MobBase,
    entity_data: steel_utils::locks::SyncMutex<WitherSkeletonEntityData>,
}

unsafe impl DowncastType for WitherSkeletonEntity {
    const TYPE_KEY: DowncastTypeKey = DowncastTypeKey::new("steel:entity/wither_skeleton");
}
...
impl WitherSkeletonEntity {
    ...
    fn new_with_base(base: EntityBase, entity_type: EntityTypeRef) -> Self {
        ...
        Self { base, entity_type, living_base, mob_base, entity_data: steel_utils::locks::SyncMutex::new(entity_data) }
    }
}
...
impl Entity for WitherSkeletonEntity {
    ...
    fn dimensions_for_pose(&self, _pose: EntityPose) -> EntityDimensions { self.entity_type.dimensions.scale(LivingEntity::get_scale(self)) }
    fn synced_data(&self) -> Option<&dyn EntitySyncedData> { Some(&self.entity_data) }
    fn save_additional(&self, nbt: &mut NbtCompound) { self.save_mob(nbt); }
    fn load_additional(&self, nbt: BorrowedNbtCompoundView<'_, '_>) { self.load_mob(nbt); }
}
...
impl LivingEntity for WitherSkeletonEntity {
    ...
    fn set_health(&self, health: f32) { let max = self.get_max_health(); self.entity_data.lock().living_entity_mut().health.set(health.clamp(0.0, max)); }
    fn server_ai_step(&self) { Mob::mob_server_ai_step(self); }
    fn ai_step(&self) -> Option<crate::entity::MoveResult> { self.default_ai_step() }
}
...
impl Mob for WitherSkeletonEntity {
    ...
    fn finalize_spawn(&self, world: &std::sync::Arc<World>, spawn_reason: EntitySpawnReason, group_data: Option<SpawnGroupData>) -> Option<SpawnGroupData> { self.finalize_spawn_mob_base(world, spawn_reason, group_data) }
