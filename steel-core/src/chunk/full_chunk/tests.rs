...
fn insertion_rejects_an_entity_owned_by_another_chunk() {
    ...
    // A jigsaw block has a vanilla block entity type but no Steel factory, so setting
    // the state leaves the position pending instead of concrete.
    let pending_block = vanilla_blocks::JIGSAW.default_state();
    assert!(
        chunk
            .set_block_state(local_pos, pending_block, UpdateFlags::UPDATE_NONE)
            .is_some()
    );
...
fn stale_no_entity_promotion_cannot_consume_a_replacement_marker() {
    ...
    // A jigsaw block has a vanilla block entity type but no Steel factory, so it stays
    // pending and cannot consume the replacement marker.
    let pending_block = vanilla_blocks::JIGSAW.default_state();
    assert_eq!(
        chunk.set_block_state(pos, pending_block, UpdateFlags::UPDATE_NONE),
        Some(moving_piston)
    );
