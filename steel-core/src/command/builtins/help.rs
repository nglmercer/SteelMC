...
fn help_list(context: &SteelCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
    ...
    const COMMANDS: &[&str] = &[
        "attribute", "clear", "clone", "deop", "damage", "difficulty", "domain", "enchant", "effect",
        "help", "me", "execute", "experience", "xp", "fill", "fly", "gamemode", "gamerule", "give",
        "kick", "kill", "list", "locate", "msg", "tell", "w", "op", "playsound", "perms", "return",
        "rotate", "save-all", "save-off", "save-on", "say", "seed", "setblock", "setworldspawn",
        "spawnpoint", "stop", "stopsound", "summon", "teleport", "tp", "tellraw", "tick", "time",
        "title", "weather", "worldborder", "invsee",
    ];
...
fn help_for_command(context: &SteelCommandContext<CommandSource>) -> Result<i32, CommandSyntaxError> {
    ...
    let message = TextComponent::plain(format!("Help for /{command_name}: try /{command_name} with TAB completion for arguments"));
    source.send_success(&message, false);
    Ok(1)
}
